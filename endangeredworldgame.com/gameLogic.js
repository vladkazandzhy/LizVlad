...
$(document).ready(function () {
  ...
  function randomizeRobotName() {
    ...
    $("#tokenExists").text(
      "There is already a token here, so you can't play anything. Click to continue with " +
        robotName +
        "'s turn."
    );
...
=======
  this.destroyToken = function (tileNum, player) {
    console.log(
      "Destroying token of player " + player + " on tile #" + tileNum + "."
    );
>>>>>>> 1e60e25de9906a7908c9f1353df1bbf3922b4fdb
...
function takeTurn(rand) {
  ...
  if (typeof rand != "undefined") {
    ...
    // if it's anything else, resume gameplay as normal
      ...
      if (playerNum == 0) {
        $("#turnDisplay").html(
          "<p>What would you like to play on tile <b>" + rand + "</b>?"
        );
        displayTurnChoices();
      } else {
        $("#turnChoices").hide();
        ...
        if (!extraTurnComp) {
          $("#turnDisplay").html(
            "<p>" +
              robotName +
              " played the following on tile <b>" +
              rand +
              "</b>. Click the deck to continue.</p>"
          );
        } else {
          $("#turnDisplay").html(
            "<p>" +
...
$("#playDefenderRule3").click(function () {
  ...
  $("#playDefenderRule3").hide();
  $("#turnDisplay").text(
    "You played a defender. Click the deck to continue with your next turn."
  );
});
...
$("#seeChanges").click(function () {
  ...
  if (playerNum == 0) {
    $("#turnDisplay").html(
      "<p>Click below to continue with " + robotName + "'s turn.</p>"
    );
    $("#continue").show();
  } else {
    $("#turnDisplay").html("<p>Click the deck to continue with your turn.</p>");
...
=======
  for (let i = 0; i < highlighted.length; i++) {
    let nearbyTile = $("#" + highlighted[i]);
    if (
      nearbyTile.hasClass("player0") &&
      !nearbyTile.hasClass("defendedbyplayer0")
    ) {
      players[0].destroyToken(highlighted[i], 0);
    } else if (
      nearbyTile.hasClass("player1") &&
      !nearbyTile.hasClass("defendedbyplayer1")
    ) {
      players[1].destroyToken(highlighted[i], 1);
    }
  }
...
function calculateRobotChoice(tileNum) {
  ...
  if (tile.hasClass("occupied")) {
    if (!extraTurnComp) {
      $("#turnDisplay").html(
        "<p>This space was already occupied, so " +
          robotName +
          " did nothing. Click the deck to continue with your turn.</p>"
      );
    } else {
      $("#turnDisplay").html(
        "<p>This space was already occupied, so " +
...
=======
  ...
  if (!$("#" + tile).hasClass("animal")) {
    $("#turnDisplay").html(
      "<p>" +
        robotName +
        " chose to do nothing on tile <b>" +
        tile +
        "</b>. Click the deck to continue with your turn.</p>"
    );
    $("#turnChoicesComp").hide();
    return "nothing";
  }
...
function countPotentialNearbyAnimals(surSpaces) {
  ...
  for (i = 0; i < surSpaces.length; i++) {
    ...
    if (
      tile.hasClass("animal") &&
      !tile.hasClass("occupied") &&
...
function checkAnimalPoints(animalId, playerId) {
  ...
  for (var i = 0; i < 100; i++) {
    if (
      $("#" + i).hasClass("a" + animalId) &&
      $("#" + i).hasClass("player" + playerId)
    ) {
...
function countFinalScore() {
  ...
  if (animalsWonByPlayer.length > 0) {
    for (var i = 0; i < animalsWonByPlayer.length; i++) {
      ...
      $("#userWon").append(
        generateAnimalLink(animalScore, animalsWonByPlayer[i])
      );
  ...
  if (animalsWonByComp.length > 0) {
    for (var i = 0; i < animalsWonByComp.length; i++) {
      ...
      $("#compWon").append(
        generateAnimalLink(animalScore, animalsWonByComp[i])
      );
    }
  }
  ...
  $("#userWon").append(
    "<p>+ <b>" + playerBombScore + "</b> points for unused bombs</p>"
  );
  $("#compWon").append(
    "<p>+ <b>" + compBombScore + "</b> points for unused bombs</p>"
  );
  ...
  $("#resultsSummary").html(
    msg +
      "<h2>Your final score: <b>" +
      playerFinalScore +
      "</b><br>" +
      robotName +
      "'s final score: <b>" +
      compFinalScore +
      "</b></h2><p><i>Click the <img class='help-icon' src='images/help-icon.png'> icons to learn more about the animals.</i></p>"
  );
}
...
function generateAnimalLink(animalScore, animalId) {
  ...
  let code =
    '<img class="animalResults" id="animal' +
    animalId +
    '" src="images/animals/' +
    animalId +
    '.jpg">';
  code +=
    "<p><b>" +
    animalScore +
    '</b> points <a href="#" title="' +
    description +
    '"><img class="help-icon" src="images/help-icon.png"></a></p>';
...
function getAnimalDescription(animalId) {
  ...
  switch (animalId) {
    case 1:
      description =
        "<b><u>Asian Elephant</u></b><br>Elephants are right- and left-tusked, and the dominant tusk is usually smaller due to greater use. Female Asian elephants have very small tusks that are hardly visible.";
      break;
    case 2:
      description =
        "<b><u>Plains Bison</u></b><br>The hump on a bison's back is made of muscle and supported by long back bones. This hump helps the bison be able to plow through snow with its head.";
      break;
    case 3:
      description =
        "<b><u>Black Rhino</u></b><br>Black rhinos have poor eyesight and sometimes charge at rocks, trees, and even trains, thinking they are threats. But their sense of smell and hearing are excellent!";
      break;
    case 4:
      description =
        "<b><u>Pygmy Hippopotamus</u></b><br>Pygmy hippos are able to sleep underwater. They have a special reflex that lets them rise to the surface, take a breath, and sink back down&mdash;all without waking up!";
      break;
    case 5:
      description =
        "<b><u>Hawaiian Monk Seal</u></b><br>After giving birth, a female Hawaiian monk seal will not eat for 5-6 weeks while she nurses her pup, causing her to lose hundreds of pounds.";
      break;
    case 6:
      description =
        "<b><u>Giant Panda</u></b><br>Giant pandas primarily eat bamboo&mdash;between 20 and 40 pounds a day! They eat for more than 12 hours a day, napping for 2-4 hours between meals.";
      break;
    case 7:
      description =
        "<b><u>African Wild Dog</u></b><br>When hunting, African wild dogs share the kill without fighting among themselves, giving priority to pups, the elderly, and the injured.";
      break;
    case 8:
      description =
        "<b><u>Bonobo</u></b><br>Bonobos share 98.7% of their DNA with humans. They are able to understand language, play musical instruments, and use various tools.";
      break;
    case 9:
      description =
        "<b><u>Amur Leopard</u></b><br>Amur leopards have thick, pale fur to help them stay warm and blend in with snow. They can run up to 37 mph and jump more than 10 feet high and 19 feet far.";
      break;
    case 10:
      description =
        "<b><u>Red Panda</u></b><br>Red pandas use their bushy tails to maintain balance when climbing trees. They also wrap their tails around them to stay warm in the winter.";
      break;
    case 11:
      description =
        "<b><u>Black-footed Ferret</u></b><br>Black-footed ferrets are nocturnal and primarily eat prairie dogs. One ferret can eat more than 100 prairie dogs in a single year!";
      break;
    case 12:
      description =
        "<b><u>Giant Tortoise</u></b><br>Giant tortoises can go up to a year without eating or drinking because of their slow metabolism.";
      break;
    case 13:
      description =
        "<b><u>Southern Sea Otter</u></b><br>Sea otters' nostrils and ears close when they swim, and they can hold their breath for up to 5 minutes.";
      break;
    case 14:
      description =
        "<b><u>Tasmanian Devil</u></b><br>The Tasmanian devil's bite is one of the strongest in the world. They can even bite through metal!";
      break;
    case 15:
      description =
        "<b><u>Iberian Lynx</u></b><br>Lynxes have very sharp vision. They can detect a mouse from up to 250 feet away!";
      break;
  }
...
function handleQuestion(tileNum, id) {
  ...
  if (playerNum == 0) {
    $("#turnDisplay").html(
      "<p>You drew a Twists & Turns card at tile <b>" +
        tileNum +
        "</b>.</p><p>Click below to read the card.</p>"
    );
  } else {
    $("#turnDisplay").html(
      "<p>" +
        robotName +
        " drew a Twists & Turns card at tile <b>" +
        tileNum +
        "</b>.</p><p>Click below to read the card.</p>"
    );
  }
...
function q1(tileNum) {
  let qText =
    "An epidemic has struck! All tokens (both yours and others players') are destroyed in the two spaces to the left and the two spaces to the right of the question tile if not protected by defenders.";
  ...
  if (playerNum == 0) {
    if (boardChanged) {
      ...
      msg +=
        "<p>No tokens were destroyed. Click below to continue with " +
        robotName +
        "'s turn.</p>";
      $("#continue").show();
    }
  ...
  // if it's the robot turn, still explain what is happening, prompt user to view changes
    ...
    if (boardChanged) {
      ...
      msg +=
        "<p>No tokens were destroyed. Click the deck to continue with your turn.</p>";
      $("#continue").hide();
      $("#pickNumber").addClass("cardHighlight");
    }
  }
...
function q2(tileNum) {
  let qText =
    "The animal population is growing! You may increase the value of any one of your tokens on the gameboard by replacing it with a higher-valued coin if you have one. If you already had a bronze or silver coin in that space, return it to your possession.";
  $("#questionCard").text(qText);
  currentQuestion = 2;
  ...
  if (playerNum == 0) {
    ...
    if (highlighted.length === 0) {
      msg +=
        "<p>You have no valued tokens that can be increased in value. Click below to continue with " +
        robotName +
        "'s turn.</p>";
      $("#turnDisplay").html(msg);
    }
    // if there is at least one valid tile option
      ...
      msg +=
        "<p>Please select which token you want to increase the value of, or click continue if you don't want to select anything.</p>";
      ruleForClicking = 2;
  ...
  // FOR COMP
    ...
    if (highlighted.length === 0) {
      msg +=
        "<p>" +
        robotName +
        " has no valued tokens that can be increased in value. Click the deck to continue with your turn.</p>";
      $("#pickNumber").addClass("cardHighlight");
    }
    // if there is at least one valid tile option
    else {
      ...
      // otherwise, move on without doing anything
        ...
        msg +=
          "<p>" +
          robotName +
          " chose to do nothing. Click the deck to continue with your turn.</p>";
      }
...
function q3(tileNum) {
  let qText =
    "The government has funded another animal reserve for your opponent. The next player has won a defender token and may place it in the current space or keep it for later.";
  $("#questionCard").text(qText);
  ...
  if (playerNum === 0) {
    ...
    if (shouldPlayDefenderVar) {
      ...
      msg +=
        "<p>It played a Defender token. Click below to continue with " +
        robotName +
        "'s turn.</p>";
    } else {
      msg +=
        "<p>It decided not to play the Defender here. Click below to continue with " +
        robotName +
        "'s turn.</p>";
    }
    ...
    msg =
      "<p>Click below to play the defender on tile " +
      tileNum +
      ". If you'd like to keep the defender for later, click the deck to continue with your turn.</p>";
...
function q4(tileNum) {
  let qText =
    "Congratulations! You have won a gold coin from an anonymous donor. Now you can you can save more animals!";
  $("#questionCard").text(qText);
...
function q5(tileNum) {
  let qText =
    "A wealthy family has transferred money to your opponent's endangered animal fund. The next player has won a silver coin!";
  $("#questionCard").text(qText);
...
function q6(tileNum) {
  let qText =
    "Congratulations! You've won a free round-trip flight. Travel to any space on the gameboard that isn't occupied and place any token (except a bomb).";
  $("#questionCard").text(qText);
  currentQuestion = 6;
  ...
  if (playerNum === 0) {
    ...
    msg +=
      "<p>Please select the space where you would like to place a token.</p>";
    ruleForClicking = 6;
  ...
  // FOR COMP
    ...
    msg +=
      ...
      " on tile number " +
      compChoiceNum +
      ". Click the deck to continue with your turn.</p>";
...
=======
  let qText =
    "A government leader has issued your opponent a private jet and has asked you to assist them. Place one of the next player's single tokens (worth 1 point) on any animal figure on the board that isn't occupied.";
  $("#questionCard").text(qText);
  ...
  if (playerNum == 0) {
    ...
    // prompt the user to click on one of these highlighted spaces
    msg +=
      "<p>Please select the space where you would like to place your opponent's token.</p>";
    ruleForClicking = 7;
  ...
  // FOR COMP
    ...
    // else choose randomly
      ...
      compChoiceNum =
        highlighted[Math.floor(Math.random() * highlighted.length)];
    }
    ...
    msg +=
      "<p>" +
      robotName +
      " has placed one of your plain tokens on tile " +
      compChoiceNum +
      ". Click the deck to continue with your turn.</p>";
...
function q8(tileNum) {
  let qText =
    "A one-month expedition of your organization was successful. You may place any valued token on any animal in this row if the space is not occupied.";
  $("#questionCard").text(qText);
  currentQuestion = 8;
  ...
  if (playerNum == 0) {
    ...
    if (highlighted.length > 0) {
      ...
      // prompt the user to click on one of these highlighted spaces
      msg +=
        "<p>Please select the space where you would like to place a token, or click Do Nothing to not place anything.</p>";
      ruleForClicking = 8;
    ...
    // if not, inform the user and prompt them to continue with the robot's turn
      ...
      msg +=
        "<p>There are no unnoccupied animal figures in the current row. Click below to continue with " +
        robotName +
        "'s turn.</p>";
      $("#continue").html("Continue");
    }
  ...
  // FOR COMP
    ...
    // if not, inform the user and prompt them to continue with their turn
      ...
      msg +=
        "<p>There are no unnoccupied animal figures in the current row, so the " +
        robotName +
        " did nothing. Click the deck to continue with your turn.</p>";
    }
...
function q9(tileNum) {
  let qText =
    "This space can't be occupied because of hazardous waste. Move two spaces up or down. You may place any token (including a bomb) in one of these spaces if not a question tile. If another player's token is already there and not protected by a defender, you may replace it with your own token of any value.";
  $("#questionCard").text(qText);
  currentQuestion = 9;
  ...
  if (playerNum == 0) {
    ...
    if (highlighted.length > 0) {
      ...
      msg +=
        "<p>There are no valid places to play a token. Click below to continue with " +
        robotName +
        "'s turn.</p>";
      $("#continue").html("Continue");
    }
  ...
  // FOR COMP
    ...
    for (let i = 0; i < highlighted.length; i++) {
      ...
      if (!tile.hasClass("occupied") && !tile.hasClass("question")) {
        ...
        tile.hasClass("player0") &&
        !tile.hasClass("defendedbyplayer0")
      ) {
        compChoices.push(highlighted[i]);
        userToken = true;
      }
    ...
    if (compChoices.length > 0) {
      ...
      if (toPlay != "nothing") {
        ...
        msg +=
          "<p>" +
          robotName +
          " chose not to play anything. Click the deck to continue with your turn.</p>";
      }
    } else {
      msg +=
        "<p>" +
        robotName +
        " wasn't able to play anything. Click the deck to continue with your turn.</p>";
    }
...
function q10(tileNum) {
  let qText =
    "You're feeling adventurous! Move three spaces up or down. If not occupied, you may place any token (including a bomb) in one of these spaces. If occupied, you may place any token in the original space.";
  $("#questionCard").text(qText);
  currentQuestion = 10;
...
function q11(tileNum) {
  let qText =
    "You have been granted unlimited authority in this area. You may place any token (except a bomb) on any space in the current column. If another person's token is occupying the space you'd like (and it isn't protected by a defender) you may discard that token and place your own instead.";
  $("#questionCard").text(qText);
  currentQuestion = 11;
...
function q12(tileNum) {
  let qText =
    "All the members of your team have suddenly become ill, so you've lost your next turn.";
  $("#questionCard").text(qText);
  ...
  if (playerNum === 0) {
    extraTurnComp = true;
    msg =
      "<div id='robotTurnMsg'><p>Click below to have " +
      robotName +
      " take its first turn.</p><button class='buttonHover' type='button' id='compFirstTurn'>Continue</button></div>";
  } else {
    extraTurnUser = true;
    $("#turnChoices").hide();
  ...
  $("body").on("click", "#compFirstTurn", function () {
    ...
    $("#robotTurnMsg2").show();
    $("#robotTurnMsg2").html(
      "<p>Click below to have " +
        robotName +
        " take its second turn.</p><button class='buttonHover' type='button' id='compSecondTurn'>Continue</button></div>"
    );
  });
...
function q13(tileNum) {
  let qText =
    "Poachers have attacked! All tokens (both yours and others players') are destroyed in the two spaces above and the two spaces below the question tile if not protected by defenders.";
  $("#questionCard").text(qText);
...
function q14(tileNum) {
  let qText =
    "Your team has discovered valuable information about how to help the animals in this area. You may place any token (except a bomb) in any space surrounding the current question tile.";
  $("#questionCard").text(qText);
  currentQuestion = 14;
  ...
  for (i = 0; i < surSpaces.length; i++) {
    if (
      !$("#" + surSpaces[i]).hasClass("occupied") &&
      !$("#" + surSpaces[i]).hasClass("question")
    ) {
...
function q15(tileNum) {
  let qText =
    "Your opponent's team has become stranded in the wilderness, so the next player loses their next turn.";
  $("#questionCard").text(qText);
  ...
  if (playerNum === 0) {
    ...
    msg =
      "<p>Click below to have " +
      robotName +
      " take another turn.</p><button class='buttonHover' type='button' id='compTurnAgain'>Continue</button>";
  }
...
function q16(tileNum) {
  let qText =
    "Volunteers have arrived to help you further your mission. Move three spaces to the right or to the left. If not occupied, you may place any token (including a bomb) in one of these spaces. If occupied, you may place any token in the original space.";
  $("#questionCard").text(qText);
...
function q17(tileNum) {
  let qText =
    "A pack of wolves has attacked some of the bison. You must remove a valued token from the bison figure if you have one placed there that isn't protected by a defender.";
  $("#questionCard").text(qText);
...
function q18(tileNum) {
  let qText =
    "Hunters seeking ivory have entered the elephants' territory. You must remove a valued token from the elephant figure if you have one placed there that isn't protected by a defender.";
  $("#questionCard").text(qText);
...
function destroyTokenOnAnimal(
  ...
  if (playerNum === 0) {
    ...
    // if there aren't, inform user and prompt them to move on
      ...
      msg +=
        "<p>You have no undefended tokens on the " +
        animalName +
  ...
  // FOR COMP
    ...
    if (undefendedComp.length > 0) {
      ...
      msg +=
        "<p>" +
        robotName +
        " has no undefended tokens on the " +
        animalName +
        ". Click the deck to continue with your turn.</p>";
    }
    $("#turnChoices").hide();
...
function q19(tileNum) {
  let qText =
    "Your opponent keeps getting one step ahead of you! The next player may place any token (except a bomb) in any space surrounding the current question tile.";
  $("#questionCard").text(qText);
  currentQuestion = 19;
  ...
  if (playerNum == 1) {
    if (highlighted.length > 0) {
      ...
      // prompt the user to click on one of these highlighted spaces
      msg +=
        "<p>Please select the space where you would like to place a token, or click the deck to not place anything and continue to your turn.</p>";
      ruleForClicking = 19;

      // this then jumps to the end of the code, search for: if (ruleForClicking == 19)
    } else {
      msg +=
        "<p>There are no valid places to play a token. Click the deck to continue with your turn.</p>";
    }
    ...
    if (compChoices.length > 0) {
      ...
      if (toPlay != "nothing") {
        ...
        msg +=
          "<p>" +
          robotName +
          " chose not to play anything. Click below to move on to " +
          robotName +
          "'s turn.</p>";
      }
    } else {
      msg +=
        "<p>" +
        robotName +
        " wasn't able to play anything. Click below to move on to " +
        robotName +
        "'s turn.</p>";
    }
...
function q20(tileNum) {
  let qText =
    "Your opponent has made the decision to cooperate more with other wildlife organizations. The next player discards one of their bombs if they have one. ";
  $("#questionCard").text(qText);
  ...
  if (playerNum === 0) {
    ...
    if (players[1].bomb > 0) {
      msg +=
        "<p>" +
        robotName +
        " has discarded a bomb.</p><p>Click below to continue.</p>";
      players[1].loseBomb();
    } else {
      msg +=
        "<p>" +
        robotName +
        " didn't have any bombs, so nothing was discarded.</p><p>Click below to continue.</p>";
    }
    $("#continue").html("Continue");
    $("#continue").show();
  }
  // FOR COMP
    ...
    if (players[0].bomb > 0) {
      msg +=
        "<p>One of your bombs has been discarded.</p><p>Click the deck to continue with your turn.</p>";
      players[0].loseBomb();
    } else {
      msg +=
        "<p>You didn't have any bombs, so nothing was discarded.</p><p>Click the deck to continue with your turn.</p>";
    }
    $("#turnChoices").hide();
    $("#pickNumber").addClass("cardHighlight");
  }
...
$("body").on("click", ".highlight", function () {
  ...
  if (ruleForClicking == 2) {
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Select which coin to play on this tile instead, or select a different tile. Or click Do Nothing to not select anything.</p>";
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Select what to place on this tile, or select a different one.</p>";
    displayTurnChoices();
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Click below to place a single token from " +
      robotName +
      " and continue to its turn, or select a different one.</p><button class='buttonHover' id='playRobotToken' onclick='playRobotToken()'>Place " +
      robotName +
      "'s token</button>";
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Select what to place on this tile, or select a different tile. Or click Do Nothing to not select anything.</p>";
    displayTurnChoices();
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Click below to remove this token, or select a different one.</p><button class='buttonHover' id='removeToken' onclick='removeToken(" +
      this.id +
      ")'>Remove Token</button>";
    ...
    msg =
      "<p>You have chosen tile <b>" +
      this.id +
      "</b>. Select what to place on this tile, or select a different tile. Or click the deck to not select anything.</p>";
    msg +=
      "<p>As soon as you make a choice, the game will continue to your turn.</p>";
...
function removeToken(tileNum) {
  ...
  $("#continue").html("Continue");
  $("#turnDisplay").html(
    "<p>Your token has been removed. Click below to continue with " +
      robotName +
      "'s turn.</p>"
  );
}
